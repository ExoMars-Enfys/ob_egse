...
LOG_PATH = Path(r"C:\wdir\EB\EB_EGSE\RS422if_log\RS422if_2026-05-18_09-48-14.log")
...
def build_hk_arrays(hk_packets):
    temp_keys = ["OB_DIGITAL", "OB_DETECTOR", "OB_MECHANISM", "OB_MOTOR",
                 "EB_MCU", "EB_PSU_BOARD", "EB_INTERNAL_TRP", "EB_PELTIER"]
...
def filter_sci_to_acq_windows(sci_datetimes, swir_low, swir_med, swir_high,
                               mwir_low, mwir_med, mwir_high, acq_windows,
                               margin_s=20.0):
    ...
    channels = [list(swir_low), list(swir_med), list(swir_high),
                list(mwir_low), list(mwir_med), list(mwir_high)]
...
def _extract_sci_series(sci_data):
    ...
    if sci_points:
        ...
        return {"abs_steps": abs_steps, "swir_low": swir_low, "swir_med": swir_med,
                "swir_high": swir_high, "mwir_low": mwir_low, "mwir_med": mwir_med,
                "mwir_high": mwir_high}
    if not hasattr(sci_data, "ABS_STEPS"):
        return None
    return {"abs_steps": [int(sci_data.ABS_STEPS)], "swir_low": [int(sci_data.SWIR_LOW)],
            "swir_med": [int(sci_data.SWIR_MED)], "swir_high": [int(sci_data.SWIR_HIGH)],
            "mwir_low": [int(sci_data.MWIR_LOW)], "mwir_med": [int(sci_data.MWIR_MED)],
...
def build_sci_arrays(sci_packets, acq_configs_list=None):
    ...
    for packet_idx, packet in enumerate(sci_packets):
        ...
        print(f"  Packet {packet_num}/{len(sci_packets)}: {num_points} points, "
...
class ClickHandler:
    def __init__(self, sci_datetimes, swir_low, swir_med, swir_high,
                 mwir_low, mwir_med, mwir_high, packet_boundaries, ax_swir, ax_mwir):
    ...
    def on_click(self, event):
        ...
        if event.inaxes is self.ax_swir:
            dists = [abs(self.swir_low[idx] - click_y),
            ...
            dists = [abs(self.mwir_low[idx] - click_y),
...
def detect_hk_anomalies(
    hk_timestamps, temp_data, volt_data,
    gap_threshold=2.0,
    ...
    temp_jumps = {
        frac: find_jumps(temp_data, hk_timestamps, frac, exclude=exclude_temp_channels)
        for frac in jump_fracs
    }
    volt_jumps = {
        frac: find_jumps(volt_data, hk_timestamps, frac)
        for frac in jump_fracs
    }
...
def _print_jump_section(label, jumps_by_frac, unit, val_fmt):
    ...
    for (ch, ts), (val, delta, hit_fracs) in sorted(seen.items(), key=lambda x: x[0][1]):
        ...
        print(f"  {ts.strftime('%H:%M:%S')}  {ch}: {val_fmt(val)}{unit}  "
...
def _draw_all_axes(ax_temp, ax_volt, ax_swir, ax_mwir, fig,
                   hk_timestamps, temp_data, volt_data, gaps, temp_jumps, volt_jumps,
                   sci_datetimes, swir_low, swir_med, swir_high,
                   mwir_low, mwir_med, mwir_high, packet_boundaries):
    ...
    for frac in sorted(temp_jumps.keys()):
        ...
        for ch, entries in temp_jumps[frac].items():
            ax_temp.scatter([e[0] for e in entries], [e[1] for e in entries],
                            marker="x", color=colour, s=sz, linewidths=1.5, zorder=5)
    ...
    for frac in sorted(volt_jumps.keys()):
        ...
        for ch, entries in volt_jumps[frac].items():
            ax_volt.scatter([e[0] for e in entries], [e[1] for e in entries],
                            marker="x", color=colour, s=sz, linewidths=1.5, zorder=5)
    ...
    if sci_datetimes:
        ...
        for start_idx, _end, packet_num, _ts in packet_boundaries:
            ax_swir.axvline(x=sci_datetimes[start_idx], color="red", linestyle="--",
                            linewidth=1, alpha=0.5)
            ax_swir.text(sci_datetimes[start_idx], 0.95, f"{packet_num}",
                         transform=ax_swir.get_xaxis_transform(),
                         rotation=90, va="top", ha="right", fontsize=8, color="red", alpha=0.7)
    ...
    if sci_datetimes:
        ...
        for start_idx, _end, packet_num, _ts in packet_boundaries:
            ax_mwir.axvline(x=sci_datetimes[start_idx], color="red", linestyle="--",
                            linewidth=1, alpha=0.5)
            ax_mwir.text(sci_datetimes[start_idx], 0.95, f"{packet_num}",
...
def main():
    ...
    if acq_configs:
        ...
        for cfg in acq_configs:
            ...
            print(f"  {cfg['timestamp'].strftime('%H:%M:%S')}  Mode={cfg['mode']} ({mode_name}),"
    ...
    (sci_datetimes, swir_low, swir_med, swir_high,
     mwir_low, mwir_med, mwir_high, packet_boundaries) = build_sci_arrays(sci_packets, acq_configs)
    ...
    (swir_low, swir_med, swir_high,
     mwir_low, mwir_med, mwir_high) = filter_sci_to_acq_windows(
        sci_datetimes, swir_low, swir_med, swir_high,
        mwir_low, mwir_med, mwir_high, acq_windows)

    # ── Figure: 4 subplots, all sharing the same datetime x-axis ─────────────
    fig, (ax_temp, ax_volt, ax_swir, ax_mwir) = plt.subplots(
    ...
    def _reload(_event=None):
        ...
        if acq_wins:
            ...
            print("No acquisition windows found in HK — showing all science data")
        sl, sm, sh, ml, mm, mh = filter_sci_to_acq_windows(
            sd, sl, sm, sh, ml, mm, mh, acq_wins)

        _draw_all_axes(ax_temp, ax_volt, ax_swir, ax_mwir, fig,
                       ts, td, vd, g, tj, vj, sd, sl, sm, sh, ml, mm, mh, pb)
