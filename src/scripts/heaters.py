...
def mech_auto_heater_test(port):
    ...
    event_log.info(
        f"Mech Heater On threshold : Resistance: 823 Ohms, Temp: -45 degrees C"
        + f"Setpoint : {(tc.hk_request(port)).THRM_MECH_ON_SP}"
    )
    event_log.info(
        f"Mech Heater Off threshold : Resistance: 882 Ohms, Temp: -30 degrees C"
    ...
    try:
        ...
        event_log.info(
            f"Turn Resistance below 823Ohm / 1848DN to trigger Auto On Mechanism Heater and press Return to confirm"
        )
        # TODO keyboard.wait("return")  # Wait for user to press Return
        ...
        if (tc.hk_request(port)).THRM_STATUS == 65:
            ...
            raise ValueError(
                event_log.error(f"Mech Auto Heaters failed to turn ON : {tc.hk_request(port).THRM_STATUS}")
            )
    ...
    try:
        ...
        event_log.info(
            f"Turn Resistance above 882Ohm / 1946DN to trigger Auto Off Mechanism Heater and press Return to confirm"
        )
        # TODO keyboard.wait("return")  # Wait for user to press Return
        event_log.info(f"Current Mech TRP: {tc.hk_request(port).MECH_TRP}")
        if (tc.hk_request(port)).THRM_STATUS == 1:
            ...
            raise ValueError(
                event_log.error(f"Mech Auto Heaters failed to turn Off : {tc.hk_request(port).THRM_STATUS}")
            )
    except ValueError as e:
