...
def outer_cal(port):
    ...
    while  not hk.MTR_FLAGS.OUTER and not hk.MTR_FLAGS.CAL and hk.MTR_ABS_STEPS != 1000 and timeout <= const.LTM_HOMING_TIMEOUT:
        time.sleep(1)
        timeout += 1
        hk = repeat(port,tc.hk_request,port)
        event_log.info(f"Calibrating to the OUTER, waiting for switch to be pressed. - Motor relative steps: {hk.MTR_REL_STEPS}")
...
def outer_home(port):
    ...
    hk = repeat(port,tc.hk_request,port)
    timeout = 1
    while  not hk.MTR_FLAGS.OUTER  and hk.MTR_ABS_STEPS not in const.LTM_OUTER_TOL   and timeout <= const.LTM_HOMING_TIMEOUT:
        time.sleep(1)
        timeout += 1
        hk = repeat(port,tc.hk_request,port)
        event_log.info(f"Moving to the Outer, waiting for switch to be pressed. - Motor absolute steps: {hk.MTR_ABS_STEPS}")
    event_log.info("Motor movement finished")
...
def base_home(port):
    ...
    while  not hk.MTR_FLAGS.BASE  and hk.MTR_ABS_STEPS not in const.LTM_BASE_TOL   and timeout <= const.LTM_HOMING_TIMEOUT:
        ...
        timeout += 1
        hk = repeat(port,tc.hk_request,port)
        event_log.info(f"Moving to the Base, waiting for switch to be pressed. - Motor absolute steps: {hk.MTR_ABS_STEPS}")
    event_log.info("Motor movement finished")
