...
SFT_NAME_RE = re.compile(r"\bSFT\s*\d+\b", re.IGNORECASE)
EXTRACTOR_VERSION = "2026.08.19-state-audit-corrected"
...
def _hk_tec_at_setpoint(hk: Any) -> bool:
...
def detect_state_condition_times(hk_packets: list[Packet]) -> dict[str, datetime]:
    ...
    if intervals and intervals[0][2] is not None:
        result["State6 Initial"] = intervals[0][2]

    # These predicates mirror verify_power_state().  Revisions 1-6 performed
    # States 3/4/5 before the first acquisition; revisions 7+ performed them
    # after it.  Search the physical state sequence rather than imposing one
    # script revision's acquisition ordering.
    state3 = first_matching(
        lambda hk: (
            _hk_operating_state(hk) == 0x04 and _hk_boards_enabled(hk) and (_hk_tec_current_a(hk) or 0.0) < 0.2
        ),
        after=state2 or standby,
    )
    ...
    state4 = first_matching(
        lambda hk: (
            _hk_operating_state(hk) == 0x04 and _hk_boards_enabled(hk) and (_hk_tec_current_a(hk) or 0.0) > 1.0
        ),
        after=state3,
    )
    ...
    state5 = first_matching(
        lambda hk: (
            ...
            and _hk_boards_enabled(hk)
            and (_hk_tec_current_a(hk) or 0.0) > 1.0
            and not _hk_heaters_commanded(hk)
        ),
...
def _sft_directory(log: Path) -> Path | None:
    """Return the nearest ancestor named for an SFT run."""
    return next(
        (parent for parent in (log.parent, *log.parents) if SFT_NAME_RE.search(parent.name)),
        None,
    )


def _run_name_for_log(log: Path) -> str:
    sft_directory = _sft_directory(log)
    return sft_directory.name if sft_directory is not None else (log.parent.name or log.stem)


def group_logs_by_sft(logs: Iterable[Path]) -> list[list[Path]]:
    """Keep every RS422 fragment from one SFT on a single decoded timeline."""
    groups: dict[tuple[str, str], list[Path]] = {}
    for log in logs:
        sft_directory = _sft_directory(log)
        # Without an SFT-labelled ancestor, keep each file independent rather
        # than accidentally combining unrelated runs in a common Log_files dir.
        key = (
            ("sft", str(sft_directory.resolve()).lower())
            if sft_directory is not None
            else ("file", str(log.resolve()).lower())
        )
        groups.setdefault(key, []).append(log)
    batches = [
        sorted(
            batch,
            key=lambda item: (parse_filename_time(item) or datetime.min, str(item).lower()),
        )
    ...
    return sorted(
        batches,
        key=lambda batch: (
            parse_filename_time(batch[0]) or datetime.min,
            str(batch[0]).lower(),
        ),
    )
...
def summarise(paths: Path | Iterable[Path], decoder: Any, mode: str) -> Summary:
    """Summarise one complete SFT, which may span several RS422 log files."""
    source_paths = (
        [paths]
    ...
    setpoint_row = _tec_at_setpoint_row(hk_packets, rows)
    if setpoint_row is not None:
        for label in ("Peltier Temp(°C)", "TEC Current (A)"):
            if label in setpoint_row:
                values[label] = setpoint_row[label]
...
def metadata_for(
    ...
    output: dict[str, Any] = {"run": _run_name_for_log(log)}
...
def _read_psu_channel_samples(path: Path, channel: str) -> list[tuple[datetime, float, float]]:
    ...
    status_dual = re.compile(
        rf"^{timestamp_pattern}\s+-\s+"
        rf"CH3\(status=\d+\)\s+(?P<ch3v>{number})(?:V)?\s+(?P<ch3i>{number})(?:A)?\s+"
        rf"CH4\(status=\d+\)\s+(?P<ch4v>{number})(?:V)?\s+(?P<ch4i>{number})(?:A)?",
        re.IGNORECASE,
    )
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        stripped = line.strip()
        match = status_dual.search(stripped) or dual.search(stripped)
        try:
            if match:
...
def _sft_number(row: tuple[Path, Summary, dict[str, Any]]) -> int | None:
    ...
    candidates: list[str] = []
    for source in (metadata, summary.values):
        if isinstance(source, dict):
            run_name = source.get("run")
            if run_name is not None:
                candidates.append(str(run_name))
    if log is not None:
        candidates.append(str(log.parent.name or log.stem))
...
def group_rows_by_test_matrix(
    ...
    """Select the preferred SFT per matrix cell but retain any valid non-matrix SFTs."""
    ...
    for eb_setpoint, ob_setpoint, preferences in TEST_MATRIX_GROUPS:
        ...
        grouped.append((log, grouped_summary, combined.copy()))

    for sft in sorted(rows_by_sft):
        if sft in preferred_sfts:
            continue
        log, summary, metadata = rows_by_sft[sft][0]
        combined = {**summary.values, **metadata}
        grouped_summary = Summary(
            values=combined,
            hk_count=summary.hk_count,
...
def _progress_update(progress: Any | None, percent: float, status: str, detail: str = "") -> None:
    if progress is None:
        print(f"[{percent:3.0f}%] {status}")
        if detail:
            print(detail)
        return
    progress.set(percent, status, detail)


def _show_warning(progress: Any | None, title: str, message: str) -> None:
...
def _show_error(progress: Any | None, title: str, message: str) -> None:
    if progress is not None:
...
def _show_info(progress: Any | None, title: str, message: str) -> None:
    if progress is not None:
...
def parse_cli_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarise RS422 thermal logs into an Excel workbook.")
    parser.add_argument("--root", type=Path, help="Folder containing RS422, PSU, MSC1, MSC2 and TP1000 logs.")
    parser.add_argument(
        "--output", type=Path, help="Optional output workbook path. Defaults to <root>/EM-EQM_Thermal_Summary.xlsx."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    selected = args.root.resolve() if args.root else select_inputs()
    if selected is None:
        return 0
    logs_root = selected.resolve()
    output = (args.output or logs_root / "EM-EQM_Thermal_Summary.xlsx").resolve()
    progress = None if args.root is not None else ProgressDialog()
    try:
        _progress_update(progress, 2, "Loading packet decoder", "Using the project EB packet utility.")
        decoder = import_decoder(None)
        _progress_update(progress, 5, "Searching for RS422 logs", str(logs_root))
        ...
        if not logs:
            ...
            return 1
        _progress_update(progress, 10, "Loading chamber and ROV temperature data", str(logs_root))
        ...
        log_batches = group_logs_by_sft(logs)
        rows: list[tuple[Path, Summary, dict[str, Any]]] = []
        for index, batch in enumerate(log_batches, 1):
            log = batch[0]
            _progress_update(
                progress,
                25 + 63 * (index - 1) / max(1, len(log_batches)),
                f"Processing SFT run {index} of {len(log_batches)}",
                f"{_run_name_for_log(log)} ({len(batch)} RS422 file(s))",
            )
            result = summarise(batch, decoder, "median")
            ...
            print(
                f"[{index}/{len(log_batches)}] {_run_name_for_log(log)}: "
                f"RS422={len(batch)}, HK={result.hk_count}, "
        ...
        _progress_update(progress, 100, "Extraction complete", state_note)
    except Exception as exc:
        if progress is not None:
            progress.close()
        _show_error(progress, "RS422 import failed", str(exc))
        return 1
