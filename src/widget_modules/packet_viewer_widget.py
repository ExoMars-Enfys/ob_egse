    ...
    async def plot_selected_sci_packet(self) -> None:
        ...
        with ui.dialog() as plot_dialog:
            with ui.card().classes("w-[95vw] max-w-6xl max-h-[90vh] overflow-auto"):
                ...
                dialog_state = {"index": max(0, min(initial_index, len(packets) - 1))}

                if not is_ob_science or ob_stitched_mode:
                    ...
                    packet_label = ui.label(f"{len(packets)} OB science points (capturing)").classes("font-bold")
                ...
                async def render_packet_plot(packet_index: int) -> None:
                    if is_ob_science:
                        if ob_stitched_mode:
                            ...
                            capture_label = getattr(packet, "CAPTURE_LABEL", "OB Science Scan")
                            packet_label.set_text(
                                f"Scan {packet_index + 1} / {len(packets)} "
                                f"(PKT {packet_number}, {point_count} points) — {capture_label}"
                            )
                            packets_to_plot = [packet]
                            title_prefix = f"{capture_label} (PKT {packet_number})"
                        else:
                            packets_to_plot = packets
                            title_prefix = "OB Science Scan (capturing)"
                    else:
                        packet = packets[packet_index]
