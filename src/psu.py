...
def close_psu_comms(psu_com: serial.Serial) -> None:
    if psu_com:
        ...
        psu_com.close()
        event_log.info(f"Closed PSU COM Port {psu_com.port}")
    return

def psuLinkCheck(psu_com) :
    if psu_com:
        try : 
            psu_com.write(f"*IDN? \r\n".encode('utf-8'))
            response = psu_com.read(8).decode('utf-8')
            if len(response) ==0 :
                raise ConnectionError(f"No response from PSU on {psu_com.port}")            
        except ConnectionError as e:
            psu_log.error(f"PSU Link Error: {e}")
