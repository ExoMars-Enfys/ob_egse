...
def run_fft() -> None:
    ...
    if not (80.0 <= ch4_current_ma <= 90.0):
        raise AssertionError(f"Initial FFT verification failed: PSU_EB_I out of range (got {ch4_current_ma:.2f} mA, expected 80-90)")
    if int(getattr(latest_hk, "CURRENT_OPERATING_STATE", -1) or -1) != 0x02:
        raise AssertionError(f"Initial FFT verification failed: CURRENT_OPERATING_STATE not SAFE (got {getattr(latest_hk, 'CURRENT_OPERATING_STATE', None)})")
    if int(getattr(latest_hk, "TCS_REJECTED", 0) or 0) != 0:
        raise AssertionError(f"Initial FFT verification failed: TCS_REJECTED not zero (got {getattr(latest_hk, 'TCS_REJECTED', None)})")
    ...
    if not (100 <= ch4_current_ma <= 110):
        raise AssertionError(f"Initial FFT verification failed: PSU_EB_I out of range (got {ch4_current_ma:.2f} mA, expected 100-110)")
    if int(getattr(latest_hk, "CURRENT_OPERATING_STATE", -1) or -1) != 0x04:
        raise AssertionError(f"Initial FFT verification failed: CURRENT_OPERATING_STATE not STANDBY (got {getattr(latest_hk, 'CURRENT_OPERATING_STATE', None)})")
    if int(getattr(latest_hk, "TCS_REJECTED", 0) or 0) != 0:
        raise AssertionError(f"Initial FFT verification failed: TCS_REJECTED not zero (got {getattr(latest_hk, 'TCS_REJECTED', None)})")
    ...
    # Check if mechanism heater is ON using the same flag as the thermal status metric (THRM_STATUS.HMS)
    try:
        latest_hk = const.hk_queue.get(timeout=2.0)
        latest_psu = const.psu_queue.get(timeout=2.0)
    except Empty as exc:
    ...
    if not (hasattr(latest_hk, "THRM_STATUS") and getattr(latest_hk.THRM_STATUS, "HDS", 0)):
        raise AssertionError(f"Initial FFT verification failed: Detector heater is  ON (THRM_STATUS.HDS={getattr(latest_hk.THRM_STATUS, 'HDS', 0)})")
