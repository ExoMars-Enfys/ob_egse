...
def power_control(port, pwr_stat, verify=True):
    ...
    if (pwr_stat < 0) or (pwr_stat > 0x03):
        tc_log.error(f"Power_Control command power_status out of limits. Rejected by EGSE {pwr_stat}")
        return
    ...
    if parsed.PWR_STAT != pwr_stat:
        tc_log.error(f"Response does not match value. Got {parsed.Param0}, expected {pwr_stat}")
...
def heater_control(
    ...
    if parsed.HTR_STAT != param:
        tc_log.error(f"Response does not match value. Got {parsed.Param0}, expected {param}")


def set_mech_sp(port, thrm_mech_off_sp, thrm_mech_on_sp, verify: bool = True):
    ## --- Check input parameters before sending CMD ---
    if (thrm_mech_off_sp < 0) or (thrm_mech_off_sp > 0xFFF):
        tc_log.error(f"Set_Mech_SP command thrm_mech_off_sp out of limits. Rejected by EGSE {thrm_mech_off_sp}")
        return

    if (thrm_mech_on_sp < 0) or (thrm_mech_on_sp > 0xFFF):
        tc_log.error(f"Set_Mech_SP command thrm_mech_on_sp out of limits. Rejected by EGSE {thrm_mech_on_sp}")
        return
    ...
    if parsed.THRM_MECH_OFF_SP != thrm_mech_off_sp:
        tc_log.error(f"ACK mech_off_sp does not match command. Set: x{thrm_mech_off_sp}, Got {parsed.THRM_MECH_OFF_SP}")

    if parsed.THRM_MECH_ON_SP != thrm_mech_on_sp:
        tc_log.error(f"ACK mech_on_sp does not match command. Set: x{thrm_mech_on_sp}, Got {parsed.THRM_MECH_ON_SP}")
...
def set_detec_sp(port, thrm_detec_off_sp, thrm_detec_on_sp, verify: bool = True):
    ...
    if (thrm_detec_off_sp < 0) or (thrm_detec_off_sp > 0xFFF):
        tc_log.error(f"Set_Detec_SP command thrm_detec_off_sp out of limits. Rejected by EGSE {thrm_detec_off_sp}")
        return

    if (thrm_detec_on_sp < 0) or (thrm_detec_on_sp > 0xFFF):
        tc_log.error(f"Set_Detec_SP command thrm_detec_on_sp out of limits. Rejected by EGSE {thrm_detec_on_sp}")
        return
    ...
    if parsed.THRM_DETEC_OFF_SP != thrm_detec_off_sp:
        tc_log.error(
            f"ACK detec_off_sp does not match command. Set: x{thrm_detec_off_sp}, Got {parsed.THRM_DETEC_OFF_SP}"
        )

    if parsed.THRM_DETEC_ON_SP != thrm_detec_on_sp:
        tc_log.error(f"ACK detec_on_sp does not match command. Set: x{thrm_detec_on_sp}, Got {parsed.THRM_DETEC_ON_SP}")
...
def set_mtr_param(port, peak_current, pwm_rate, speed, pwm_duty, verify=True):
    ...
    if (pwm_rate < 0) or (pwm_rate > 0xFFFF):
        tc_log.error(f"Set_MTR_Param command pwm rate out of limits. Rejected by EGSE {pwm_rate}")
    ...
    if parsed.MTR_CURRENT != peak_current:
        tc_log.error(f"ACK peak current not as commanded. Set: x{peak_current:04X}, Got: x{parsed.MTR_CURRENT:04X}")

    if parsed.MTR_PWM_RATE != pwm_rate:
        tc_log.error(f"ACK pwm_rate not as commanded. Set: x{pwm_rate:04X}, Got: x{parsed.MTR_PWM_RATE:04X}")

    if parsed.MTR_SPEED != speed:
        tc_log.error(f"ACK HK speed not as commanded. Set: x{speed:04X}, Got: x{parsed.MTR_SPEED:04X}")

    if parsed.MTR_PWM_DUTY != pwm_duty:
        tc_log.error(f"ACK HK pwm_duty not as commanded. Set: x{pwm_duty:04X}, Got: x{parsed.MTR_PWM_DUTY:04X}")
...
def set_mtr_guard(port, recirc, guard, recval, spisel, verify=True):
    ...
    if parsed.MTR_RECIRC != recirc:
        tc_log.error(f"ACK recirc not as commanded. Set: x{recirc:04X}, Got: x{parsed.RECIRC:04X}")

    if parsed.MTR_GUARD != guard:
        tc_log.error(f"ACK guard not as commanded. Set: x{guard:04X}, Got: x{parsed.GUARD:04X}")

    if parsed.MTR_RECVAL != recval:
        tc_log.error(f"ACK recval not as commanded. Set: x{recval:04X}, Got: x{parsed.MTR_RECVAL:04X}")

    if parsed.MTR_SPISEL != spisel:
        tc_log.error(f"ACK spi_sel not as commanded. Set: x{spisel:04X}, Got: x{parsed.MTR_SPISEL:04X}")
...
def mtr_mov_pos(port, pos_steps, verify=True):
    ...
    if (pos_steps < 0) or (pos_steps > 0x3200):
        tc_log.error(f"Move Pos Steps command pos_steps out of limits. Rejected by EGSE {pos_steps}")
        return
...
def mtr_mov_neg(port, neg_steps, verify=True):
    ...
    if (neg_steps < 0) or (neg_steps > 0x3200):
        tc_log.error(f"Move Neg Steps command pos_steps out of limits. Rejected by EGSE {neg_steps}")
        return
...
def mtr_mov_abs(port, abs_pos, verify=True):
    ...
    if (abs_pos < 0) or (abs_pos > 0x3200):
        tc_log.error(f"Move Neg Steps command pos_steps out of limits. Rejected by EGSE {abs_pos}")
        return
